...
var dbPath = path.resolve('/tmp/', 'last_body.txt');
var fbSleep = {};
...
function dump_body(body){
    console.error('Body is invalid, dumped to a file');
    try {
...
fbSleep.getLastActiveTimes = function(config) {
    ...
    .then(function(body) {
        var match = body.match(/lastActiveTimes\":({.+?})/);
        if (!match){
            dump_body(body);
        }

        var lastActiveTimes = JSON.parse(match[1]);
